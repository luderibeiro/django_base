...
def validate_project_slug(slug):
    ...
    if not re.match(r"^[a-z][a-z0-9_]*$", slug):
        print(
            "❌ Error: Project slug must start with a letter and contain only lowercase letters, numbers, and underscores."
        )
        sys.exit(1)
    return slug
...
def main():
    ...
    if (
        "{{ cookiecutter.use_redis }}" == "y"
        and "{{ cookiecutter.cache_backend }}" != "redis"
    ):
        print(
            "⚠️  Warning: You selected Redis but not as cache backend. This might cause issues."
        )

    if (
        "{{ cookiecutter.use_postgresql }}" == "y"
        and "{{ cookiecutter.database_engine }}" != "postgresql"
    ):
        print(
            "⚠️  Warning: You selected PostgreSQL but not as database engine. This might cause issues."
        )
